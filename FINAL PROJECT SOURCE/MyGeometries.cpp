...
void renderTempleLawn() {
    ...
    float mapVerts[] = {
        ...
        8.0f, 12.0f, 15.0f,      0.0f, 0.0f, 1.0f,          mapValx - 0.30f, mapValy+0.0f,         // Back left
        16.0f, 12.0f, 15.0f,      0.0f, 0.0f, 1.0f,         mapValx, mapValy +0.0f,         // Back right
        16.35f, 9.0f,  15.0f,      0.0f, 0.0f, 1.0f,        mapValx, mapValy - 0.55f,         // Front right
        8.2f, 9.0f,  15.0f,      0.0f, 0.0f, 1.0f,          mapValx - 0.30f, mapValy - 0.55f,         // Front left
    };
    ...
    float mapfVerts[] = {
        // Position              // Normal                  // Texture coordinates
        -8.2f, 12.0f, 15.0f,      0.0f, 0.0f, 1.0f,          mapValx - 0.30f, mapValy +0.1f,         // Back left
        8.0f, 12.0f, 15.0f,      0.0f, 0.0f, 1.0f,           mapValx, mapValy + 0.1f,         // Back right
        8.6f, 2.0f,  15.0f,      0.0f, 0.0f, 1.0f,           mapValx, mapValy - 0.45f,         // Front right
        -8.8f, 2.0f,  15.0f,      0.0f, 0.0f, 1.0f,          mapValx - 0.30f, mapValy - 0.45f,         // Front left
    };
...
void renderBuilding3() {
    ...
    matDemo = viewMatrix;
    matDemo.Mult_glTranslate(-18 - snorlaxPOSx, 2.0, -10 - snorlaxPOSy);
    matDemo.Mult_glScale(1.5, 1.5, 1.5);
    matDemo.DumpByColumns(matEntries);           // Apply the model view matrix
    glUniformMatrix4fv(modelviewMatLocation, 1, false, matEntries);
    glBindTexture(GL_TEXTURE_2D, TextureNames[22]);     // Choose Earth image texture
    glUniform1i(applyTextureLocation, true);           // Enable applying the texture!
    texSphere.Remesh(4, 4);
    texSphere.Render();

    matDemo = viewMatrix;
    matDemo.Mult_glTranslate(-15 - snorlaxPOSx, 1.8, -8 - snorlaxPOSy);
    matDemo.Mult_glScale(1.5, 1.5, 1.5);
    matDemo.DumpByColumns(matEntries);           // Apply the model view matrix
    glUniformMatrix4fv(modelviewMatLocation, 1, false, matEntries);
    glBindTexture(GL_TEXTURE_2D, TextureNames[23]);     // Choose Earth image texture
    glUniform1i(applyTextureLocation, true);           // Enable applying the texture!
    texSphere.Render();
